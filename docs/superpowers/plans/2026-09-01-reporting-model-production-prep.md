...
**Architecture:** Keep the existing reporting pipeline and make four focused changes: derive the recipient from the selected account, use the evaluated model bundle through environment-overridable defaults, retry malformed structured responses with bounded cost, and keep all evaluation inputs and results in the ignored private directory. Public tests and docs use generic examples only.
...
- [ ] Replace client-specific constants and examples with generic values.
- [ ] Derive the private evaluation account name and recipient from Meta insight rows, then store them only in the ignored fixture.
- [ ] Run the approved client-identifier patterns across tracked files while excluding `.private-evals`; expect no results.
- [ ] Rewrite the remote evaluation branch with one sanitized commit based on `origin/main`.
...
- [ ] Write failing tests for control-character removal, generic/wrong greetings, missing greetings, and Meta-row precedence.
- [ ] Run `npm test -- tests/reporting-recipient.test.ts`; expect failure before implementation.
- [ ] Add a pure helper that returns an exact, normalized recipient handle and repairs only the opening line.
- [ ] Pass the selected account name from the UI, prefer Meta's account name when insight rows exist, and apply the helper before judging, returning, logging, or sending.
- [ ] Run `npm test -- tests/reporting-recipient.test.ts`; expect all tests to pass.
...
- [ ] Add failing tests proving malformed JSON is retried once, required keys are enforced, retry costs are included, and blank threshold variables use defaults 8 and 7.
- [ ] Run `npm test -- tests/reporting-eval-controls.test.ts`; expect the new tests to fail.
- [ ] Keep report summary and judges on `openai/gpt-5.4-mini`; default tone extraction to `openai/gpt-5.6-luna`; default client composition to `openai/gpt-5.6-terra` with tested fallbacks.
- [ ] Bound structured-response attempts to two per model, retry only response-shape failures, and expose every attempt in usage logs.
- [ ] Remove the unavailable model ID from the default chain.
- [ ] Fix empty threshold parsing so missing variables use the documented defaults.
- [ ] Run `npm test -- tests/reporting-eval-controls.test.ts`; expect all tests to pass.
...
- [ ] Upgrade `next` and `eslint-config-next` from `16.2.4` to patched stable `16.3.4`.
- [ ] Replace local absolute paths with repository-relative links.
- [ ] Update architecture, model policy, privacy boundary, known limits, and rollback instructions without naming any client or including IDs, keys, prompts, or results.
- [ ] Apply the copy-editing checklist: clarity, consistent voice, sourced claims, concrete wording, formatting, and working links.
...
- [ ] Run unit tests, lint, production build, and `npm audit --omit=dev`.
- [ ] Run the monthly finalist evaluation with the existing frozen private data and hard budget stop; do not fetch Meta again unless the fixture is invalid.
- [ ] Confirm summary facts, exact recipient, voice, JSON shape, latency, and cost for three candidate runs.
- [ ] Run a repository and full-history privacy scan with redacted output plus GitHub secret-scanning alerts.
- [ ] Run the branch security review and code review; fix verified findings, then repeat affected checks.
